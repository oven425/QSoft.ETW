...
public static class EtlAnalyzer
{
    ...
    private static AnalysisResult Analyze(string etlPath, CancellationToken cancellationToken)
    {
        ...
        // EtlFileReader 目前改為在解析過程中即時觸發事件(ThreadCSwitch、PerfInfoDPC/ISR 等)，
        // 而非在 ProcessFile 結束後回傳完整的分析物件。因此以 EtlEventAggregator 訂閱這些事件，
        // 在事件發生當下即時累計統計資料，並在 ProcessFile 完成後彙總成 AnalysisResult。
        var aggregator = new EtlEventAggregator();
        AnalysisResult result;
        try
        {
            var reader = new EtlFileReader();
            aggregator.Attach(reader);
